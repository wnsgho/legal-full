...
class KnowledgeGraphExtractor:
            ...
            try:
                with open(output_file, "w", encoding='utf-8') as output_stream:
                    for batch in data_loader:
                        ...
                        # Write results
                        # print(f"Processed {batch_counter} batches ({batch_counter * self.config.batch_size_triple} chunks)")  # 로그 제거
                ...
                with open(output_file, "w", encoding='utf-8') as output_stream:
                    for batch in data_loader:
                        ...
                        # Write results with ASCII fallback
                        # print(f"Processed {batch_counter} batches ({batch_counter * self.config.batch_size_triple} chunks)")  # 로그 제거
                        for i in range(len(batch_ids)):
                            result = self.prepare_result_dict(batch_data, stage_outputs, i)
                            ...
                            # ASCII 모드로 저장 (유니코드 문자 제거)
                            safe_result = json.dumps(result, ensure_ascii=True) + "\n"
                            output_stream.write(safe_result)
                            output_stream.flush()

    def convert_json_to_csv(self):
        # 필요한 디렉토리 생성
        os.makedirs(f"{self.config.output_directory}/triples_csv", exist_ok=True)
        ...
        # CSV to GraphML 변환 시도 (인코딩 오류 처리)
        try:
            ...
            print(f"⚠️ CSV to GraphML 변환 중 오류 발생: {e}")
            print("🔄 이 단계는 건너뛰고 계속 진행합니다.")
    ...
    def generate_concept_csv_temp(self, batch_size: int = None, **kwargs):
        ...
        os.makedirs(f"{self.config.output_directory}/concept_csv", exist_ok=True)
