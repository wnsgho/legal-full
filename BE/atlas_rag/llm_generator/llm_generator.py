...
class LLMGenerator():
    def __init__(self, client, model_name, backend='openai', verbose=False):
    ...
    def _api_inference(self, message, max_new_tokens=8192,
        ...
        if self.verbose:
            print(f"🔍 API 호출 시작 - 모델: {self.model_name}")
            print(f"🔍 메시지 수: {len(message)}")
            print(f"🔍 max_tokens: {max_new_tokens}, temperature: {temperature}")
            print(f"🔍 모델명 소문자: {self.model_name.lower()}")
            print(f"🔍 o4 포함 여부: {'o4' in self.model_name.lower()}")
            print(f"🔍 o4-mini 포함 여부: {'o4-mini' in self.model_name.lower()}")
        
        try:
            # o1, o4 모델들은 max_completion_tokens를 사용하고 temperature 제한이 있음
            # o4-mini-2025-04-16 모델도 포함
            if ('o1' in self.model_name.lower() or 'o4' in self.model_name.lower() or 
                'o4-mini' in self.model_name.lower() or self.model_name == 'o4-mini-2025-04-16'):
                # o1, o4 모델들은 temperature를 1로 고정하거나 제거
                response = self.client.chat.completions.create(
                        model=self.model_name,
                        messages=message,
                        max_completion_tokens=max_new_tokens,
                        temperature=1.0,  # o1, o4 모델들은 temperature를 1로 고정
            ...
            if self.verbose:
                print(f"✅ API 호출 성공 - 소요시간: {time_cost:.2f}초")
            
            content = response.choices[0].message.content
            if self.verbose:
                print(f"🔍 원본 content: {repr(content)}")
                print(f"🔍 response.choices[0].message 속성들: {dir(response.choices[0].message)}")
            
            if content is None and hasattr(response.choices[0].message, 'reasoning_content'):
                content = response.choices[0].message.reasoning_content
                if self.verbose:
                    print(f"🔍 reasoning_content 사용: {repr(content)}")
            
            # o4 모델의 경우 다른 응답 구조일 수 있음
            if content is None or content == "":
                if self.verbose:
                    print("🔍 content가 None이거나 빈 문자열, 다른 속성 확인 중...")
                # 다른 가능한 속성들 확인
                message_obj = response.choices[0].message
                for attr in ['text', 'message', 'response', 'output']:
                    if hasattr(message_obj, attr):
                        alt_content = getattr(message_obj, attr)
                        if alt_content and alt_content != "":
                            content = alt_content
                            if self.verbose:
                                print(f"🔍 {attr} 속성에서 content 발견: {repr(content)}")
                            break
                
            validate_function = kwargs.get('validate_function', None)
            if validate_function:
                if self.verbose:
                    print(f"🔍 validate_function 적용 중...")
                content = validate_function(content, **kwargs)
                if self.verbose:
                    print(f"🔍 검증 후 content: {repr(content)}")
            elif self.verbose:
                print(f"🔍 validate_function 없음, 원본 content 사용")
                
        except Exception as e:
            # 상세 로그 제거 - 간단한 오류 메시지만 출력
            if self.verbose:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                def process_message(i):
                    try:
                        ...
                        print(f"❌ LLM API 호출 중 예외 발생: {str(e)}")
    ...
    def large_kg_ner(self, text, simple_ner = False):
        ...
        try:
            ...
            # Handle different return types from json_repair.loads()
            if isinstance(cleaned_data, dict) and 'keywords' in cleaned_data:
                return cleaned_data['keywords']
            elif isinstance(cleaned_data, list):
                return cleaned_data
            else:
                print(f"⚠️ 예상치 못한 JSON 구조: {type(cleaned_data)}")
                return []
