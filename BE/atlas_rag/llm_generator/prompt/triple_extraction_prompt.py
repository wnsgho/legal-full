TRIPLE_INSTRUCTIONS = {
    ...
    "ko": {
        "system": "당신은 항상 유효한 JSON 배열 형식으로 응답하는 도우미입니다",
        "entity_relation": """주어진 문단에서 모든 중요한 엔티티와 그들 간의 관계를 간결하게 요약하세요. 관계 설명은 엔티티 간의 연결을 명확하게 표현해야 하며, 헤드와 테일 엔티티의 정보를 반복하지 않아야 합니다. 엔티티는 구체적이고 명확해야 하며, 대명사는 제외해야 합니다.
        반드시 다음 JSON 형식으로 출력해야 합니다:
        [
            {
                "Head": "{명사}",
                "Relation": "{동사 또는 관계 설명}",
                "Tail": "{명사}"
            }...
        ]""",

        "event_entity": """문단의 이벤트와 그에 참여하는 엔티티를 분석하세요. 각 이벤트는 독립적인 단일 문장이어야 하며, 모든 관련 엔티티를 나열하세요(구체적이어야 하며, 대명사는 제외).
        반드시 다음 JSON 형식으로 출력해야 합니다:
        [
            {
                "Event": "{이벤트를 설명하는 간단한 문장}",
                "Entity": ["엔티티1", "엔티티2", "..."]
            }...
        ]""",
       
        "event_relation": """이벤트 간의 시간적 또는 인과적 관계를 분석하세요. 관계 유형은 다음과 같습니다: 이전, 이후, 동시에, 때문에, 결과로. 각 이벤트는 독립적인 단일 문장이어야 합니다.
        반드시 다음 JSON 형식으로 출력해야 합니다:
        [
            {
                "Head": "{이벤트1 설명}",
