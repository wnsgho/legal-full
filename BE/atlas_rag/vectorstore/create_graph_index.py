...
def create_embeddings_and_index(sentence_encoder, model_name: str, working_directory: str, keyword: str, include_events: bool, include_concept: bool,
    ...
    text_list = [node for node in tqdm(node_list, disable=True) if "passage" in KG.nodes[node]["type"]]
    
    if not include_events and not include_concept:
        node_list = [node for node in tqdm(node_list, disable=True) if "entity" in KG.nodes[node]["type"]]
    elif include_events and not include_concept:
        node_list = [node for node in tqdm(node_list, disable=True) if "event" in KG.nodes[node]["type"] or "entity" in KG.nodes[node]["type"]]
    elif include_events and include_concept:
        node_list = [node for node in tqdm(node_list, disable=True) if "event" in KG.nodes[node]["type"] or "concept" in KG.nodes[node]["type"] or "entity" in KG.nodes[node]["type"]]
    else:
