...
import { AlertTriangle, Info, Eye, Filter, RefreshCw } from "lucide-react";
...
interface RiskClausesProps {
  ...
  onRiskAnalysisComplete?: (analysis: string) => void;
}
...
const RiskClauses: React.FC<RiskClausesProps> = ({
  ...
  const handleRiskAnalysis = async () => {
    setIsAnalyzing(true);
    try {
      const response = await api.autoAnalyzeRisks(
        "이 계약서의 위험조항을 종합적으로 분석해주세요. 발견된 위험요소, 위험도 평가, 영향 당사자, 개선 권고사항을 포함하여 상세히 설명해주세요."
      );

      if (response.success) {
        setRiskAnalysis(response.answer);
        if (onRiskAnalysisComplete) {
          onRiskAnalysisComplete(response.answer);
        }
        toast({
          title: "위험조항 분석 완료",
          description: "계약서의 위험조항 분석이 완료되었습니다.",
        });
      } else {
        throw new Error(response.message || "위험조항 분석에 실패했습니다.");
      }
    } catch (error) {
  ...
  const filteredClauses = riskClauses.filter((clause) => {
    const riskMatch =
      selectedRiskLevel === "all" || clause.riskLevel === selectedRiskLevel;
    const categoryMatch =
  ...
  const highlightClauseInText = (text: string, clause: RiskClause) => {
    ...
    return (
      <div className="whitespace-pre-wrap">
        {before}
        <mark
          className={`px-1 py-0.5 rounded ${getRiskLevelColor(
            clause.riskLevel
          )}`}
        >
          {highlighted}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(riskLevelCounts).map(([level, count]) => (
              <div key={level} className="text-center">
                <div
                  className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${getRiskLevelColor(
                    level as RiskLevel
                  )}`}
                >
          ...
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedRiskLevel(e.target.value as RiskLevel | "all")
                }
            ...
            <div className="flex items-center space-x-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedCategory(e.target.value as ClauseCategory | "all")
                }
                className="text-sm border rounded px-2 py-1"
              >
                <option value="all">전체</option>
                        ...
                        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
                          <DialogHeader>
                            <DialogTitle className="flex items-center space-x-2">
                              ...
                              <span>
                                {getCategoryLabel(clause.category)} 조항
                              </span>
                            </DialogTitle>
                          </DialogHeader>
